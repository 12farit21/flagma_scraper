...
PROXY_TYPE_TOR = 'tor'
PROXY_TYPE_DATAIMPULSE = 'dataimpulse'
...
class HttpRequest():
    def __init__(self, headers: dict=HEADERS, max_retries: int=MAX_RETRIES,
        ...
        self.dataimpulse_proxy = DataImpulseProxy()
        self.proxy_index = -1
    ...
    def _get_next_proxy(self):
        ...
        elif self.proxies == PROXY_TYPE_DATAIMPULSE:
            logging.info('Using DataImpulse proxy.')
            return self.dataimpulse_proxy.get_proxies()
    ...
    def rotate_proxy(self):
        ...
        if current_ip:
            logging.info(f'Now using IP: {current_ip}')
        else:
            logging.warning('Could not determine current IP address')
